...
\section{Indexare}
\emph{Indexarea} este procesul unui sistem de reg'asire a informa'tiei prin care se colecteaz'a, parseaz'a 'si stocheaz'a informa'tia pentru a facilita reg'asirea rapid'a 'si precis'a. Proiectarea unui index presupune cuno'stin'te asupra unor concepte interdisciplinare din lingvistic'a, psihologie cognitiv'a, matematic'a, informatic'a, etc. Motoarele populare de c'autare se concetreaz'a pe indexarea de tip \emph{full-text} ale documentelor online scrise 'in limbaj natural. Tipurile media(video, audio, imagini) pot fi 'si ele indexate. Motoarele de meta-c'autare refolosesc indec'sii altor servicii f'ar'a s'a stocheze un index local, 'in timp ce sistemele RI cu cache stocheaz'a permanent at'jt indexul c'jt 'si corpusul. Spre deosebire de indec'sii full-text, un index \emph{partial-text} restric'tioneaz'a "ad'jncimea" index'arii pentru a reduce dimensiunea indexului. Unele servicii efectueaz'a procesul de indexare la intervale predeterminate de timp din cauza costurilor de procesare, 'in timp ce alte motoarele de c'autare indexeaz'a con'tinut 'in timp real.

\subsection{Aspecte ale proiect'arii}
Iat'a c'jteva aspecte majore 'in proiectarea arhitecturii unui sistem RI.
\paragraph{'Imbinarea} se refer'a la modul 'in care informa'tia ajunge 'in index sau cum cuvintele sunt ad'augate 'in index 'in timpul unei travers'ari a corpusului 'si la posibilitatea de a permite indexare concurent'a 'si asincron'a. Programul de indexare trebuie mai 'intai s'a verifice dac'a actualizeaz'a con'tinut vechi sau adaug'a con'tinut nou.
\paragraph{Tehnica de stocare} se refer'a la modul de stocare a datelor, mai exact la compresarea 'si filtrarea informa'tiei.
\paragraph{M'arimea indexului} reprezint'a nevoia de spa'tiu de stocare.
\paragraph{Viteza de c'autare} se refer'a la c'jt de repede poate fi g'asit un cuv'jnt 'in index. Viteza g'asirii unui element 'intr-o structur'a de date, 'impreun'a cu viteza de actualizare 'si 'stergere reprezint'a un punct central 'in informatic'a.
\paragraph{'Intre'tinerea} se refer'a la felul 'in care un index este men'tinut de-a lungul timpului.
\paragraph{Tolerarea defectelor} trateaz'a aspecte precum coruperea indexului, informa'tie eronat'a, hardware stricat, parti'tionare, replicare, etc.

\section{Metode de reg'asire 'si ierarhizare}
...
Folosire: 
\begin{quote}\small
\texttt{trec\_eval [-h] [-q] [-a] [-o] [-c] [-l<num>  [-Nnum] [-Mnum] [-Uanum] [-Ubnum] [-Ucnum] [-Udnum] [-T] trec\_rel\_file trec\_top\_file}
\end{quote}
