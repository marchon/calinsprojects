...
\section{Un exemplu}
S'a presupunem c'a avem la dispozi'tie o colec'tie de piese scrise de Shakespeare 'si vrem sa determin'am care dintre aceste piese con'tine cuvintele \emph{Brutus 'SI Caesar} 'si \emph{NU} con'tine cuv'jntul \emph{Calpurnia}. O posibilitate este s'a se parcurg'a fiecare pies'a cuv'jnt cu cuv'jnt. Acest tip de scanare liniar'a este o form'a foarte simpl'a de reg'asire a documentelor 'si poart'a numele de \emph{grepping} (de la comanda Unix). Acest proces poate fi eficient, mai ales 'in contextul vitezei calculatoarelor moderne; pentru o colec'tie de dimensiuni reduse nu este nevoie de nimic mai mult.

Dar pentru o serie de alte scopuri, este nevoie de mai mult:
\begin{enumerate}
	\item Pentru a procesa o colec'tie mare de documente rapid. Cantitatea datelor on-line a crescut cel pu'tin la fel de repede ca viteza calculatoarelor, 'si se pune problema c'aut'arii 'in colec'tii care 'insumeaz'a trilioane de cuvinte.
	\item Pentru a permite interog'ari mai flexibile. 
...
\caption{Rezultate Lucene, F2EXP 'si BM25 la rularea pe corpusul \emph{Oshumed} primul set de interog'ari, folosind doar c'jmpul \emph{Title}}
\label{fig:res1}
\end{figure}
...
\caption{Rezultate Lucene, F2EXP 'si BM25 la rularea pe corpusul \emph{Hamshahri} primul set de interog'ari, folosind doar c'jmpul \emph{Title}}
\label{fig:res2}
\end{figure}
