...
CString CPath::GetFileVersionInfoString(LPCSTR verInfoString)
{
	if(GetExtName().MakeLower().Compare("exe"))
		return CString("Must be executable!");

	LPCSTR fileFullName = (LPCSTR)_strOriginalPath;
	char *rcData = 0; 
	DWORD dwSize;
	UINT wSize;
